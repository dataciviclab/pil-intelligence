...
# --- Distribuzione PIL pro-capite per macro-area ---
...
area_cagr = trend.groupby("macro_area").agg(
    cagr_medio=("pil_cagr_pct", "mean"),
    n=("geo", "count")
).reset_index().sort_values("cagr_medio", ascending=True)
...
if "pil_first" in trend.columns and "pil_cagr_pct" in trend.columns:
...
gap_data = hub[hub["macro_area"].isin(["Nord-Ovest", "Nord-Est", "Sud", "Isole"])].copy()
if len(gap_data) > 0:
    avg = gap_data.groupby(["year", "macro_area"])["pil_procapite_eur"].mean().reset_index()
...
show = trend[["geo_label_en", "macro_area", "pil_cagr_pct", "gva_delta_pct", "emp_delta_pct"]].copy()
